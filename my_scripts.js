...
function viewStudentStats(id, toggle){
	if(toggle){
		...
		document.getElementById(id).style.visibility = "0px";
	}
}
...
function loadStatsPage(){
	...
	for (var i = 2; i < table.rows.length; i++){
		var hscore = parseInt(table.rows[i].cells[2].innerHTML);
		var oScore = parseInt(table.rows[i].cells[3].innerHTML);
		if(hscore > oScore){
			var winner = "CU!";
			wins++;
...
function switchPlayers(playerNum){
	...
	avgPY = (players[playerNum].pass_yards) / (players[playerNum].games_played);
	var a = Math.round(avgPY);
	document.getElementById("avg_p_yards").innerHTML = a;
	avgRY = (players[playerNum].rushing_yards) / (players[playerNum].games_played);
	var b = Math.round(avgRY);
	document.getElementById("avg_r_yards").innerHTML = b;
	avgREY = (players[playerNum].receiving_yards) / (players[playerNum].games_played);
	var c = Math.round(avgREY);
	document.getElementById("avg_rec_yards").innerHTML = c;
}
